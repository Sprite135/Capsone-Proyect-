...
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.CompanyProfile') AND name = 'SeaceEntityAcronym')
BEGIN
    ALTER TABLE dbo.CompanyProfile ADD SeaceEntityAcronym NVARCHAR(100) NOT NULL DEFAULT '';
    PRINT 'Added: SeaceEntityAcronym';
END

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.CompanyProfile') AND name = 'SeaceDepartment')
BEGIN
    ALTER TABLE dbo.CompanyProfile ADD SeaceDepartment NVARCHAR(100) NOT NULL DEFAULT '';
    PRINT 'Added: SeaceDepartment';
END

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.CompanyProfile') AND name = 'SeaceProvince')
BEGIN
    ALTER TABLE dbo.CompanyProfile ADD SeaceProvince NVARCHAR(100) NOT NULL DEFAULT '';
    PRINT 'Added: SeaceProvince';
END

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.CompanyProfile') AND name = 'SeaceDistrict')
BEGIN
    ALTER TABLE dbo.CompanyProfile ADD SeaceDistrict NVARCHAR(100) NOT NULL DEFAULT '';
